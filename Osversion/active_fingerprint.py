...
# ---------------- GUI Wrapper ---------------- #
def run_active_os_detection(ip, log_func=print):
    default_ports = [80, 443, 22, 21, 25]  # Common ports
    tracer = ActiveFingerprint(ip, default_ports, max_threads=5)

    original_print = print
    try:
        globals()['print'] = log_func  # Redirect print to GUI log
        tracer.run()
    finally:
